...
\pagestyle{fancy}
\fancyhf{}
\rhead{\textit{XGBoost para Riesgo Crediticio}}
...
\begin{tikzpicture}[node distance=2cm]
    \node(A) [draw, rectangle] {¿Ingreso > \$40,000?};
    \node(B) [draw, rectangle, below left of=A, xshift=-2.5cm] {¿Edad > 30?};
    \node(C) [draw, rectangle, below right of=A, xshift=2.5cm] {RIESGO BAJO};
    \node(D) [draw, rectangle, below left of=B, xshift=-2cm] {RIESGO BAJO};
    \node(E) [draw, rectangle, below right of=B, xshift=2cm] {RIESGO ALTO};
    
    \draw[->] (A) -- (B) node[midway, left] {Sí};
    \draw[->] (A) -- (C) node[midway, right] {No};
    \draw[->] (B) -- (D) node[midway, left] {Sí};
    \draw[->] (B) -- (E) node[midway, right] {No};
\end{tikzpicture}
...
\node(start) [block] {Datos del Cliente};
\node(norm) [block, below of=start] {Normalizar};
\node(feat) [block, below of=norm] {Ingeniería de Características};
\node(model) [block, below of=feat] {Pasar por 200 Árboles};
\node(vote) [block, below of=model] {Promediar Votos};
\node(threshold) [block, below of=vote] {Comparar con Umbral (11.6\%)};
\node(predict) [block, below of=threshold] {Predicción: Riesgo o No-Riesgo};

\draw[arrow] (start) -- (norm);
\draw[arrow] (norm) -- (feat);
\draw[arrow] (feat) -- (model);
\draw[arrow] (model) -- (vote);
\draw[arrow] (vote) -- (threshold);
\draw[arrow] (threshold) -- (predict);
